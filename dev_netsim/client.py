...
def encrypt(filename, command, statefile, password):
	...
	packet = header_type + header_sqn + filename

	#List remote files
	if command == 1:
		#No file or payload required by server
		encrypted = cipher.encrypt(Padding.pad(packet,AES.block_size))
	...
	#upload
		...
		try:
			print(filenameCopy)
			ifile = open(statefile[:-9] + filenameCopy, 'rb')
			payload = ifile.read()
			ifile.close()
		except:
		...
		encrypted = cipher.encrypt(Padding.pad(packet+payload,AES.block_size))
	#download
	elif command == 3:
	...
	#logout 
		...
		encrypted = cipher.encrypt(Padding.pad(packet+new_key,AES.block_size)) 
	...
	mac = MAC.digest()
	message = header_from + iv +  encrypted + mac
...
def decrypt(msg, statefile):
	...
	# parse the message
	header = msg[:1]                    # header is 6 bytes long
	iv = msg[1:(AES.block_size)+1]      # iv is AES.block_size bytes long
	encrypted = msg[(AES.block_size)+1:-32] # encypted part
	mac = msg[-32:]
	header_from = header        # from is encoded on 1 byte
	...
	header_type = decrypted[:1]
	header_sqn = decrypted[1:5]
